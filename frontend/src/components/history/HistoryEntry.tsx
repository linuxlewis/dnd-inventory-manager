...
import { formatRelativeTime, formatActionDescription, formatCurrencyChanges, formatFieldChanges } from './utils'
...
export function HistoryEntry({ entry }: HistoryEntryProps) {
  ...
  const detailText =
    entry.action === 'currency_updated'
      ? formatCurrencyChanges(entry.details)
      : entry.action === 'item_updated'
        ? formatFieldChanges(entry.details)
        : null
